...
| `unit/handlers/validate-render-checks.test.ts`      | Render-check shape guards, unavailable-Godot failure path, merging alongside structure checks                                                       |                                                                   |
| `unit/png-decoder.test.ts`                          | Zero-dependency PNG decoding: filter reversal, format rejection                                                                                     |                                                                   |
| `unit/render-check.test.ts`                         | Pixel statistics (chromatic/dominant/distinct) and the movie-writer flow with a stubbed spawn                                                       | Mocks `child_process.spawn`                                       |
