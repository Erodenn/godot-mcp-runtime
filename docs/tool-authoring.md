...
| Heterogeneous ordered operations on shared state in one process        | **One tool with per-item operation field is acceptable** if the discriminator is load-bearing - dropping it would force agents to chain N round-trips and lose the shared-state guarantee. `batch_scene_operations` and `validate.checks` are the documented exceptions in this server: the alternative (N singular calls) costs ~3s startup per call and loses the in-process shared state (the scene cache for the first, one instantiated scene tree for the second). **Rare.** Never use as an escape hatch from designing per-outcome tools. |
